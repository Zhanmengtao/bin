...
my $kog_script_dir = dirname($0);
my $kogFile = "$kog_script_dir/kog/kog";
my $funFile = "$kog_script_dir/kog/fun.txt";
my (%proteinid_to_kogid, %kog_to_fun, %kog_to_desc, %fun_code_to_category);

open KOG, '<', $kogFile or die $!;
$/ = "\n\n";
while (<KOG>) {
    my $kogID;
    if (/\[(\w+)\]\s+(KOG\d+)\s+(.+)\n/) {
    ...
    foreach (@lines) {
        ...
        if (s/^\s+\w+:\s+//) {
            my @proteins = split /\s+/;
            foreach (@proteins) { $proteinid_to_kogid{$_} = $kogID }
        }
    }
}
$/ = "\n";
close KOG;
print STDERR "kog File: $kogFile\n";
...
$name =~ s/.xml//;
my $xls = "$name".".xls";
my $geneannot = $name . "_kog_gene_annot.xls";
my $kogclass = $name . "_kog_class_annot.xls";
...
while ( <IN> ) {
    ...
    if ( /<\/BlastOutput>/ ) {
        ...
        foreach ( @hit ) {
            ...
            my $kog_id = $proteinid_to_kogid{$Subject_id};
            my $kog_fun = $kog_to_fun{$kog_id};
            my $Subject_annotation = $kog_to_desc{$kog_id};

            my $functional_categories;

            if ( $kog_fun ) {
                my @kog_fun = split //, $kog_fun;
                foreach ( @kog_fun ) {
            ...
            if ( $Evalue <= $E_value && $match_percentage >= $coverage ) {
                printf GENEANNOT "$Query_id\t$Subject_id\t$Score\t$Evalue\t%0.4f\t$kog_id\t$Subject_annotation\t$kog_fun\t$functional_categories\n",$match_percentage;
                printf XLS "$Query_id\t$Query_length\t$Query_start\t$Query_end\t$Subject_id\t$Subject_length\t$Subject_start\t$Subject_end\t$Query_frame\t%0.4f\t%0.4f\t%0.4f\t$Align_length\t$Score\t$Evalue\t%0.4f\t$kog_id\t$Subject_annotation\t$kog_fun\t$functional_categories\n",$Identity,$Positive,$Gap,$match_percentage;
            }
        }
        $one = "";
    }
...
open KOGCLASS, '>', $kogclass;
print KOGCLASS "Code\tFunctional-Categories\tGene-Number\tGenes\n";
foreach my $function_code ( @fun_codes ) {
    if ( exists $fun_code_to_genes{$function_code} ) {
        ...
        print KOGCLASS "$function_code\t$fun_code_to_category{$function_code}\t$gene_number\t$genes_out\n";
    }
    else {
        print KOGCLASS "$function_code\t$fun_code_to_category{$function_code}\t0\n";
    }
}
close KOGCLASS;
print "Output File 3 : $kogclass\n";
